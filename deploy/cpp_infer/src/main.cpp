...
#include "hand_out_process/hand_out_process.h"
#include "dbnet_pre_process/dbnet_pre_process.h"
#include "dbnet_infer_process/dbnet_infer_process.h"
#include "dbnet_post_process/dbnet_post_process.h"
#include "cls_pre_process/cls_pre_process.h"
#include "cls_infer_process/cls_infer_process.h"
#include "cls_post_process/cls_post_process.h"
#include "crnn_pre_process/crnn_pre_process.h"
#include "crnn_infer_process/crnn_infer_process.h"
#include "crnn_post_process/crnn_post_process.h"
#include "collect_process/collect_process.h"
#include "config_parser/config_parser.h"
#include "command_parser/command_parser.h"
#include "module_manager/module_manager.h"

using AscendBaseModule::MT_HandOutProcess;
using AscendBaseModule::MT_DbnetPreProcess;
using AscendBaseModule::MT_DbnetInferProcess;
using AscendBaseModule::MT_DbnetPostProcess;
using AscendBaseModule::MT_ClsPreProcess;
using AscendBaseModule::MT_ClsInferProcess;
using AscendBaseModule::MT_ClsPostProcess;
using AscendBaseModule::MT_CrnnPreProcess;
using AscendBaseModule::MT_CrnnInferProcess;
using AscendBaseModule::MT_CrnnPostProcess;
using AscendBaseModule::MT_CollectProcess;
...
void ModuleDescGenerator(int deviceNum, std::vector<AscendBaseModule::ModuleDesc> *moduleDesc, TaskType taskType) {
  ...
  if (taskType == TaskType::DET || taskType == TaskType::DET_REC || taskType == TaskType::DET_CLS_REC) {
    moduleDesc->push_back({MT_DbnetPreProcess, static_cast<int>(std::ceil(0.6 * deviceNum))});
    moduleDesc->push_back({MT_DbnetInferProcess, static_cast<int>(std::ceil(1 * deviceNum))});
    moduleDesc->push_back({MT_DbnetPostProcess, static_cast<int>(std::ceil(2 * deviceNum))});
  }
  if (taskType == TaskType::CLS || taskType == TaskType::DET_CLS_REC) {
    moduleDesc->push_back({MT_ClsPreProcess, static_cast<int>(std::ceil(0.6 * deviceNum))});
    moduleDesc->push_back({MT_ClsInferProcess, static_cast<int>(std::ceil(1 * deviceNum))});
    moduleDesc->push_back({MT_ClsPostProcess, static_cast<int>(std::ceil(0.6 * deviceNum))});
  }
  if (taskType == TaskType::REC || taskType == TaskType::DET_REC || taskType == TaskType::DET_CLS_REC) {
    moduleDesc->push_back({MT_CrnnPreProcess, static_cast<int>(std::ceil(0.7 * deviceNum))});
    moduleDesc->push_back({MT_CrnnInferProcess, static_cast<int>(std::ceil(1 * deviceNum))});
    moduleDesc->push_back({MT_CrnnPostProcess, static_cast<int>(std::ceil(0.26 * deviceNum))});
  }
...
void ModuleConnectDesc(std::vector<AscendBaseModule::ModuleConnectDesc> *connectDesc, TaskType taskType) {
  // det connect
  std::string preModule = MT_HandOutProcess;
  std::string lastModule;

  if (taskType == TaskType::DET || taskType == TaskType::DET_REC || taskType == TaskType::DET_CLS_REC) {
    connectDesc->push_back({preModule, MT_DbnetPreProcess, MODULE_CONNECT_RANDOM});
    ...
    lastModule = MT_DbnetPostProcess;
  }
  if (taskType == TaskType::CLS || taskType == TaskType::DET_CLS_REC) {
    connectDesc->push_back({preModule, MT_ClsPreProcess, MODULE_CONNECT_RANDOM});
  ...
  if (taskType == TaskType::REC || taskType == TaskType::DET_REC || taskType == TaskType::DET_CLS_REC) {
    ...
    lastModule = MT_CrnnPostProcess;
  }
  connectDesc->push_back({lastModule, MT_CollectProcess, MODULE_CONNECT_RANDOM});
}
