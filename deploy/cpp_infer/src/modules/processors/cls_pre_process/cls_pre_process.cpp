...
ClsPreProcess::~ClsPreProcess() = default;

Status ClsPreProcess::Init(CommandParser *options, const ModuleInitArgs &initArgs) {
...
Status ClsPreProcess::DeInit() {
  LogInfo << "cls_pre_process [" << instanceId_ << "]: Deinit success.";
  return Status::OK;
}

Status ClsPreProcess::ParseConfig(CommandParser *options) {
...
uint8_t *ClsPreProcess::PreprocessCls(const std::vector<cv::Mat> &frames, uint32_t batchSize) {
  cv::Mat resizedImg;
  cv::Mat inImg;
...
Status ClsPreProcess::Process(std::shared_ptr<void> commonData) {
