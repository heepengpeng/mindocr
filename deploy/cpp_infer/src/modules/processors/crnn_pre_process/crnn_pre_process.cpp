...
CrnnPreProcess::~CrnnPreProcess() = default;

Status CrnnPreProcess::Init(CommandParser *options, const AscendBaseModule::ModuleInitArgs &initArgs) {
...
Status CrnnPreProcess::DeInit() {
  LogInfo << "crnn_pre_process [" << instanceId_ << "]: Deinit success.";
  return Status::OK;
}

Status CrnnPreProcess::ParseConfig(CommandParser *options) {
...
int CrnnPreProcess::GetCrnnMaxWidth(const std::vector<cv::Mat> &frames, float maxWHRatio) {
  int maxResizedW = 0;
...
uint8_t *CrnnPreProcess::PreprocessCrnn(const std::vector<cv::Mat> &frames, uint32_t batchSize, int maxResizedW,
...
Status CrnnPreProcess::Process(std::shared_ptr<void> commonData) {
