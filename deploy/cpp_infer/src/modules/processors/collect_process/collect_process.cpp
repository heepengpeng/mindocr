...
CollectProcess::~CollectProcess() = default;

Status CollectProcess::Init(CommandParser *options, const AscendBaseModule::ModuleInitArgs &initArgs) {
...
Status CollectProcess::DeInit() {
  LogInfo << "collect_process [" << instanceId_ << "]: Deinit success.";
  return Status::OK;
}

Status CollectProcess::ParseConfig(CommandParser *options) {
...
Status CollectProcess::Process(std::shared_ptr<void> commonData) {
...
std::string CollectProcess::GenerateDetClsRecInferRes(const std::string &imageName, uint32_t frameSize,
...
std::string
CollectProcess::GenerateDetInferRes(const std::string &imageName, const std::vector<std::string> &inferRes) {
  std::string result;
  result.append(imageName).append("\t[");
...
std::string
CollectProcess::GenerateClsInferRes(const std::string &imageName, const std::vector<std::string> &inferRes) {
  std::string result;
  result.append(imageName).append("\t");
...
std::string
CollectProcess::GenerateRecInferRes(const std::string &imageName, const std::vector<std::string> &inferRes) {
  std::string result;
  result.append(imageName).append("\t");
  for (const auto &s : inferRes) {
    result.append(s).append(" ");
