...
Status CrnnInferProcess::Init(CommandParser *options, const ModuleInitArgs &initArgs) {
...
Status CrnnInferProcess::DeInit() {
  LogInfo << "crnn_infer_process [" << instanceId_ << "]: Deinit success.";
  return Status::OK;
}

Status CrnnInferProcess::ParseConfig(CommandParser *options, const ModuleInitArgs &initArgs) {
...
Status CrnnInferProcess::MindXModelInfer(const std::shared_ptr<CommonData> &data) {
...
Status CrnnInferProcess::LiteModelInfer(const std::shared_ptr<CommonData> &data) {
  auto srcData = data->imgBuffer;
  auto batchSize = data->batchSize;
  auto maxResizedW = data->maxResizedW;
...
Status CrnnInferProcess::Process(std::shared_ptr<void> commonData) {
