...
#include <utility>
#include <vector>
#include "dbnet_post/clipper.hpp"
#include "dbnet_post/dbnet_post.h"

DbnetPost::DbnetPost() {}

Status DbnetPost::DbnetMindXObjectDetectionOutput(const std::vector<MxBase::Tensor> &singleResult,
...
Status DbnetPost::DbnetLiteObjectDetectionOutput(const std::vector<mindspore::MSTensor> &singleResult,
                                                 std::vector<std::vector<TextObjectInfo>> *textObjInfos,
                                                 const std::vector<ResizedImageInfo> &resizedImageInfos) {
  LogDebug << "DbnetPost start to write results.";
  uint32_t batchSize = singleResult.size();
...
void DbnetPost::FindContours(std::vector<std::vector<TextObjectInfo>> *textObjInfos,
...
void DbnetPost::ConstructInfo(std::vector<TextObjectInfo> *textObjectInfo, const std::vector<cv::Point2f> &box,
...
void DbnetPost::FilterByMinSize(const std::vector<cv::Point> &contour,
                                std::vector<cv::Point2f> *box,
                                float *minSide) {
...
void DbnetPost::FilterByBoxScore(const cv::Mat &prediction, const std::vector<cv::Point2f> &box, float *score) {
...
int DbnetPost::NpClip(const int &coordinate, const int &sideLen) {
  if (coordinate < 0) {
    return 0;
  }
