...
Status DetInferNode::ParseConfig(CommandParser *options, const ModuleInitArgs &initArgs) {
  ...
  backend_ = Utils::ConvertBackendTypeToEnum(options->GetStringOption("--backend"));
  if (backend_ == BackendType::LITE) {
    dbNetLite_ = new mindspore::Model();
    mindspore::Status buildRet = mindspore::kLiteParamInvalid;
    if (Utils::EndsWith(detModelPath, ".mindir")) {
      buildRet = dbNetLite_->Build(detModelPath, mindspore::kMindIR, initArgs.context);
    } else if (Utils::EndsWith(detModelPath, ".ms")) {
      buildRet = dbNetLite_->Build(detModelPath, mindspore::kMindIR_Lite, initArgs.context);
    }
...
void func(std::vector<int64_t> path) {
...
Status DetInferNode::LiteModelInfer(std::shared_ptr<CommonData> data) {
  ...
  data->outputLiteTensorVec.clear();
  for (const auto& output : outputs) {
    data->outputLiteTensorVec.emplace_back("", output.DataType(), output.Shape(), output.Clone()->Data().get(), output.DataSize());
  }
