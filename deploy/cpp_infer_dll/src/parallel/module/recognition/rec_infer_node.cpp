...
Status RecInferNode::ParseConfig(CommandParser *options, const ModuleInitArgs &initArgs) {
  ...
  if (staticMethod_) {
    ...
    for (auto &file : files) {
      if (backend_ == BackendType::LITE && Utils::EndsWith(file, ".mindir")) {
    ...
    if (backend_ == BackendType::LITE) {
      ...
      if (Utils::EndsWith(pathExpr, ".mindir")) {
        buildRet = model->Build(pathExpr, mindspore::kMindIR, initArgs.context);
      } else if (Utils::EndsWith(pathExpr, ".ms")) {
        buildRet = model->Build(pathExpr, mindspore::kMindIR_Lite, initArgs.context);
      }
      if (buildRet != mindspore::kSuccess) {
        // LogError << "Build lite model error " << buildRet;
        return Status::LITE_MODEL_BUILD_FAILURE;
