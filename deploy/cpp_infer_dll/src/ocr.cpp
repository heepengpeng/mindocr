...
void MainProcess(CommandParser *options, const std::string &streamName, TaskType taskType) {
  ...
  std::cout << "end to end cost: " << costMs << " in ms / " << costMs / S2MS << " in s. ";
}
...
Status ParseArgs(int argc, const char *argv[], CommandParser *options) {
  ...
  options->AddOption("--backend", "lite", "Inference engine type.");
...
Status args_check(CommandParser *options, bool useCls) {
  ...
  std::string detModelPath = options->GetStringOption("--det_model_path");
  if (!detModelPath.empty()) {
    auto ret = Utils::CheckPath(detModelPath, "detModelPath");
    if (ret != Status::OK) {
...
//    }
  ...
  if (!recModelPath.empty()) {
    auto ret = Utils::CheckPath(recModelPath, "rec_model_path");
    if (ret != Status::OK) {
...
//             totalImgs << "].";
  ...
  Utils::RmDir(modelConfigPath);
  std::string splitConfigPath("./config");
  Utils::RmDir(splitConfigPath);
