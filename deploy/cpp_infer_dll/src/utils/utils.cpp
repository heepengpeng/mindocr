...
int Utils::MakeDir(const std::string &path, bool replace) {
  if (replace && access(path.c_str(), 0) != -1) {
    auto ret = RmDir(path.c_str());
    if (ret != 0) {
      return ret;
...
int Utils::RmDir(const std::string &path) {
  if (access(path.c_str(), 0) != -1) {
    std::cout << path << " exists" << std::endl;
    int ret = 0;
#ifdef _WIN32
    ret = system(("rmdir /s /q \"" + std::string(path) + "\"").c_str());
#elif __linux__
    ret = system(("rm -r " + path).c_str());
#endif
    std::cout << "RmDir" << ret << std::endl;
    if (ret != 0) {
      return ret;
    }
    return ret;
