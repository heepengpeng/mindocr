...
// list all files under a directory
    ...
    void Utility::print_result(const std::vector<OCRPredictResult> &ocr_result) {
      ...
      const auto fixed_value = std::pow(10, precision);
      auto func = [&fixed_value](float value) { return round(value * fixed_value) / fixed_value; };

      for (size_t i = 0; i < ocr_result.size(); i++) {
        std::string print_line = std::to_string(i);
        // det
        std::vector<std::vector<int>> boxes = ocr_result[i].box;
        if (!boxes.empty()) {
          json det_json(boxes);
          print_line += "\tdet boxes: " + det_json.dump();
        }

        // rec
        if (ocr_result[i].score != -1.0) {
          json rec_json({ocr_result[i].text, func(ocr_result[i].score)});
          print_line += "\trec text: " + rec_json.dump();
        }

        // cls
        if (ocr_result[i].cls_label != -1) {
          json rec_json({ocr_result[i].cls_label, func(ocr_result[i].cls_score)});
          print_line += "\tcls label: " + rec_json.dump();
        }
    ...
    std::string Utility::result_to_file(const std::vector<OCRPredictResult> &results, const std::string& res_save_dir,
                                        const std::string& file_name, bool det, bool cls, bool rec) {

      auto single_save_filepath = res_save_dir + "/" + file_name + "_predicted.txt";
      std::string collect_save_filepath;
      ...
      for (const auto& result : results) {
        if(det && !cls && !rec) {
          output_json.push_back(result.box);
          collect_save_filepath = "det_results.txt";
        } else if (!det && cls && !rec) {
          output_json = json({result.cls_label, func(result.cls_score)});
          collect_save_filepath = "cls_results.txt";
        } else if (!det && !cls && rec) {
          output_json = json({result.text, func(result.score)});
          collect_save_filepath = "rec_results.txt";
        } else if (det && rec) {
          output_json.push_back({{"transcription", result.text}, {"points", result.box}, {"score", func(result.score)}});
          collect_save_filepath = "pipeline_results.txt";
        } else if (det && cls && !rec) {
          output_json  = json({{"points", result.box}, {"cls_label", result.cls_label}, {"cls_score", func(result.cls_score)}});
          collect_save_filepath = "det_cls_results.txt";
        } else if (!det && cls && rec) {
          output_json = json({{"transcription", result.text}, {"score", func(result.score)}, {"cls_label", result.cls_label}, {"cls_score", func(result.cls_score)}});
          collect_save_filepath = "cls_rec_results.txt";
        }
      }

      auto line_res = output_json.dump() + "\n";

      std::ofstream outfile1(single_save_filepath, std::ios::out);
      outfile1 << file_name << "\t" << line_res;
      outfile1.close();

      collect_save_filepath =  res_save_dir + "/" + collect_save_filepath;
      std::ofstream outfile2(collect_save_filepath, std::ios::out | std::ios::app);
      outfile2 << file_name << "\t" << line_res;
      outfile2.close();
